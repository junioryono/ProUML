...
export function LoginProviders({ isLoading }: { isLoading: boolean }) {
   ...
   function openOAuthWindow(url: string) {
      // Open a child window to the OAuth provider
      ...
      childWindow.addEventListener("message", (event) => {
         ...
         if (event.data.payload === "failure") {
            childWindow.close();
            console.log("Failed OAuth login");
            return;
         }
   ...
   return (
      <div className="grid gap-3">
         <button
            ...
            onClick={() => openOAuthWindow("https://api.prouml.com/oauth/github/login")}
            disabled={isLoading}
         >
            <svg
         ...
         <button
            type="button"
            className="inline-flex w-full items-center justify-center rounded-lg border bg-white px-5 py-2.5 text-center text-sm font-medium text-black hover:bg-slate-100 focus:outline-none focus:ring-4 focus:ring-[#24292F]/50 disabled:opacity-50 dark:hover:bg-[#050708]/30 dark:focus:ring-slate-500"
            onClick={() => openOAuthWindow("https://api.prouml.com/oauth/google/login")}
            disabled={isLoading}
         >
