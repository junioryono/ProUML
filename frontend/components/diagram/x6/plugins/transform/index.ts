...
export class Transform extends Basecoat<Transform.EventArgs> {
   ...
   protected getTransformOptions(node: Node) {
      ...
      const resizing = Transform.parseOptionGroup<Transform.ResizingRaw>(this.graph, node, this.options.resizing);

      const rotating = Transform.parseOptionGroup<Transform.RotatingRaw>(this.graph, node, this.options.rotating);
