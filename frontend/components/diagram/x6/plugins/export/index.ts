import { DataUri, NumberExt, FunctionExt, Vector, Rectangle, Size, KeyValue, Basecoat, Dom, Graph } from "@antv/x6";
import "./api";
...
export class Export extends Basecoat<Export.EventArgs> {
   ...
   toSVG(callback: Export.ToSVGCallback, options: Export.ToSVGOptions = {}) {
      ...
      vSVG
         ...
         .attr("viewBox", [viewBox.x - 15, viewBox.y - 15, viewBox.width + 30, viewBox.height + 30].join(" "));
         ...
         raws.forEach((elem, index) => {
            ...
            Object.keys(computedStyle).forEach((property) => {
               if (
                  ...
                  computedStyle.getPropertyValue(property) !== defaultComputedStyle[property]
               ) {
   ...
   protected notify<K extends keyof Export.EventArgs>(name: K, args: Export.EventArgs[K]) {
