import { KeyValue, ObjectExt, FunctionExt, Basecoat, IDisablable, Cell, Model, Graph } from "@antv/x6";
import "./api";

export class History extends Basecoat<History.EventArgs> implements IDisablable {
   public name = "history";
   public graph: Graph;
   public model: Model;
   ...
   protected validate(events: string | string[], ...callbacks: History.Validator.Callback[]) {
   ...
   protected addCommand<T extends keyof Model.EventArgs>(event: T, args: Model.EventArgs[T]) {
      ...
      if (this.batchCommands) {
         ...
         if (this.lastBatchIndex >= 0 && (diffId || diffName)) {
            ...
            const index = this.batchCommands.findIndex(
               (cmd) => ((isModelChange && cmd.modelChange) || cmd.data.id === cell.id) && cmd.event === event,
            );
   ...
   protected notify(event: keyof History.EventArgs, cmd: History.Commands | null, options: KeyValue) {
   ...
   protected consolidateCommands() {
      ...
      if (maybePositionChange.event !== "cell:change:position" || !maybePositionChange.options?.ui) {
         return;
      }
...
export namespace History {
   ...
   export namespace Validator {
      ...
      export type Callback = (err: Error | null, cmd: History.Command, next: (err: Error | null) => any) => any;
...
namespace Util {
   ...
   export function getOptions(options: History.Options): History.CommonOptions {
      const reservedNames = ["cell:added", "cell:removed", "cell:change:*", "grid:changed", "background:changed"];

      const batchEvents = ["batch:start", "batch:stop"];

      const eventNames: any = options.eventNames
         ? options.eventNames.filter(
              (event) => !(Util.isChangeEvent(event) || reservedNames.includes(event) || batchEvents.includes(event)),
           )
         : reservedNames;
